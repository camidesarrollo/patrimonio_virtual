...
//echo "$n2: |$a2|<br>\n";
			...
			<form method="post" action="options.php">
				...
				<table style="border: 1px solid #ddd; background: #f1f1f1; width: 100%;">
					...
					<tbody id="language_tbody" style="border: 1px solid black; background: #f9f9f9;">
						...
						$count = 1;
		        		while (true) {
		        			if (trim(get_option('biblioredes_idioma_' . $count)) == '' ||
	...
	function post_metaboxes() {
		if (get_post_type() == 'post') {
			...
			if (in_array('recorridos-sonoros', $category_names) ||
...
//					console.log("i = " + i);
					for (j = 1; j < i; j++) {
						id = 'image-' + count + '-' + j;
//						console.log("images = images + document.getElementById('" + id + "').value;");
...
//echo "add_post_meta($postid, '_image-' . $suffix, 'image.jpg');";
		add_post_meta($postid, '_image-' . $suffix, 'image.jpg');
		wp_die(); // this is required to terminate immediately and return a proper response
	}
...
//echo "update_post_meta($postid, '_image-" . $count . "-" . $j . "', '" . $images[$j - 1] . "');\n";
			update_post_meta($postid, '_image-' . $count . '-' . $j, $images[$j - 1]);
		}
		wp_die(); // this is required to terminate immediately and return a proper response
	}
...
*/
	...
	function metabox_language_content($post, $args) {
        ...
        <div class="wrap">
			...
			?>
			<input type="submit" name="add-lang-entry" value="<?php echo __('Add') . ' ' . __('Post'); ?>" onclick="add_lang_entry(event, '<?php echo get_the_ID() . "', '" . $suffix; ?>');"></input>
        </div>
        <?php
	}
	...
	function print_audio_text_item($idioma_code, $count, $title_meta, $audio_meta, $text_meta) {
		...
		<tbody style="background: #f9f9f9;">
			...
			<tr>
				...
				<td style="width: 80%;">
					<textarea id="text-<?php echo $suffix; ?>" rows="5" cols="50" style="padding: 5px; margin: 5px; width: -webkit-calc(100% - 10px); width: -moz-calc(100% - 10px); -webkit-box-sizing: border-box; -moz-box-sizing: border-box; -o-box-sizing: border-box; -ms-box-sizing: border-box; box-sizing: border-box;"><?php echo $text_meta; ?></textarea>
				</td>
			</tr>
			<tr>
				<td></td>
				<td style="padding-bottom: 10px;">
					<input type="submit" name="delete-lang-entry" value="<?php echo __('Delete'); ?>" onclick="delete_lang_entry(event, '<?php echo get_the_ID() . "', '" . $suffix; ?>');"></input>
					<input type="submit" name="update-lang-entry" value="<?php echo __('Update'); ?>" onclick="update_lang_entry(event, '<?php echo get_the_ID() . "', '" . $suffix; ?>');"></input>
				</td>
			</tr>
		</tbody>
		</table>
	...
	function metabox_images_content($post, $args) {
    	...
    	<?php
    	$count = 1;
		$image = get_post_meta(get_the_ID(), "_image-$count-1", true);
    	while ($image != '') {
		...
		<input type="submit" name="add-image-entry" value="<?php echo __('Add') . ' ' . __('Post'); ?>" onclick="add_image_entry(event, <?php echo get_the_ID() . ', ' . $count; ?>);"></input>
    	<?php
	}
	...
	function print_images_entry($count) {
		...
		<tbody style="background: #f9f9f9;">
			<?php
			$i = 1;
			$image = get_post_meta(get_the_ID(), "_image-$count-$i", true);
			if ($image != '') {
				do {

					print '<tr>' . "\n" . '<td style="width: 20%;">Imagen ' . $i . "</td>\n";
					print '<td style="width: 80%;">' . "\n";
					print '<input id="image-' . $count . '-' . $i . '" size="30" value="' . $image. '" style="padding: 5px; margin: 5px; width: -webkit-calc(100% - 50px); width: -moz-calc(100% - 50px); -webkit-box-sizing: border-box; -moz-box-sizing: border-box; -o-box-sizing: border-box; -ms-box-sizing: border-box; box-sizing: border-box;"></input>' . "\n";
					$id = $count . '-' . $i;
					print '<span id="image-delete-' . $id . '" onclick="delete_image(event, ' . get_the_ID() . ', ' . $count . ', ' . $i . ');"><font size="+1"><b>X</b></font></span>' . "\n";
					print "</td>\n</tr>\n";
					$i++;
					$image = get_post_meta(get_the_ID(), "_image-$count-$i", true);
				} while ($image != '');
			...
			<tr>
				...
				<td style="padding: 7px; padding-bottom: 10px;">
					<input type="submit" name="add-image" value="<?php echo __('Add'); ?>" onclick="add_image(event, <?php echo get_the_ID() . ', ' . $count . ', ' . $i; ?>);"></input>
					<input type="submit" name="update-image-entry" value="<?php echo __('Update'); ?>" onclick="update_image_entry(event, <?php echo get_the_ID() . ', ' . $count . ', ' . $i; ?>);"></input>
				</td>
			</tr>
		</tbody>
		</table>
	...
	function metabox_map_content($post, $args) {
    	...
    	<table style="border-spacing: 0px; border: 2px solid #f1f1f1; background: #f1f1f1; width: 100%; margin-top: 20px; margin-bottom: 7px;">
			...
			print '<td style="width: 80%;">' . "\n";
$svg_file = get_post_meta(get_the_ID(), "_map_svg_file", true);
			...
			print '<td style="width: 80%;">' . "\n";
$svg_file2 = get_post_meta(get_the_ID(), "_map_svg_file2", true);
			...
			$count = 1;
			$map_elem = get_post_meta(get_the_ID(), "_map-elem-1", true);
			while ($map_elem != '') {
				...
				print '<span id="image-delete-' . $count . '" onclick="delete_map_entry(event, ' . get_the_ID() . ', ' . $count . ');"><font size="+1"><b>X</b></font></span>' . "\n";
				print "</td>\n</tr>\n";
				$count++;
				$map_elem = get_post_meta(get_the_ID(), "_map-elem-" . $count, true);
			}
			?>
			<tr>
				<td></td>
				<td style="padding: 7px; padding-bottom: 10px;">
					<input type="submit" name="add-map-entry" value="<?php echo __('Add'); ?>" onclick="add_map_entry(event, <?php echo get_the_ID() . ', ' . $count; ?>);"></input>
					<input type="submit" name="update-map-entry" value="<?php echo __('Update'); ?>" onclick="update_map_entry(event, <?php echo get_the_ID() . ', ' . $count; ?>);"></input>
				</td>
			</tr>
			</tbody>
		</table>
